...
//------------------------Parameter----------------------
`define TV_IN_A "../tv/cdatafile/c.matrix_mult.autotvin_A.dat"
`define TV_IN_B "../tv/cdatafile/c.matrix_mult.autotvin_B.dat"
`define TV_OUT_C "../tv/rtldatafile/rtl.matrix_mult.autotvout_C.dat"
parameter ADDR_WIDTH = 8;
parameter DATA_WIDTH = 32;
parameter A_DEPTH = 16;
reg [31 : 0] A_OPERATE_DEPTH = 0;
parameter A_c_bitwidth = 32;
parameter B_DEPTH = 16;
...
parameter matrix_mult_continue_addr = 0;
parameter matrix_mult_auto_start_addr = 0;
parameter A_data_in_addr = 64;
parameter B_data_in_addr = 128;
parameter C_data_out_addr = 192;
    ...
    if(fp == 0) begin       // Failed to open file
      ...
      for (i = 0; i < (C_DEPTH - C_DEPTH % factor); i = i + 1) begin
          ...
          if (factor == 2) begin
              ...
              bin_data = tmp_cache_mem;
              write_binary_C(fp,bin_data,16);
          end
          if (factor == 1) begin
              bin_data = mem_C[i];
              write_binary_C(fp,bin_data,32);
          end
      end 
      if (factor == 4) begin
          if ((C_DEPTH - 1) % factor == 2) begin
              bin_data = mem_C[C_DEPTH / factor][7:0];
              write_binary_C(fp,bin_data,8);
              bin_data = mem_C[C_DEPTH / factor][15:8];
              write_binary_C(fp,bin_data,8);
              bin_data = mem_C[C_DEPTH / factor][23:16];
              write_binary_C(fp,bin_data,8);
          end
      ...
      if (factor == 2) begin
          if ((C_DEPTH - 1) % factor == 0) begin
              bin_data = mem_C[C_DEPTH / factor][15:0];
              write_binary_C(fp,bin_data,16);
          end
      end
      transaction_idx = transaction_idx + 1;
