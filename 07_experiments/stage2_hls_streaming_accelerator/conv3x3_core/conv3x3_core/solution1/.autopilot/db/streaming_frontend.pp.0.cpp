...
# 2 "<built-in>" 2
# 1 "../streaming_frontend.cpp" 2
# 1 "../streaming_frontend.h" 1
...
# 10 "D:/Xilinx/Vitis_HLS/2022.2/common/technology/autopilot\\ap_fixed.h" 2
...
# 11 "D:/Xilinx/Vitis_HLS/2022.2/common/technology/autopilot\\ap_int.h"
...
# 2 "../streaming_frontend.cpp" 2

enum frontend_state_t {
    FRONTEND_IDLE = 0,
    FRONTEND_FILL_BUFFER = 1,
    FRONTEND_PROCESS = 2
};
...
#pragma HLS INLINE off
 ...
 static data_t windows[INPUT_CHANNELS][VALID_WINDOWS]
                        [KERNEL_SIZE][KERNEL_SIZE];

    data_t line_buf[BUFFER_ROWS][IMAGE_WIDTH];
    data_t window[KERNEL_SIZE][KERNEL_SIZE];
    data_t new_column[KERNEL_SIZE];

    VITIS_LOOP_28_1: for (int channel = 0; channel < INPUT_CHANNELS; ++channel) {
        int window_index = 0;

        VITIS_LOOP_31_2: for (int row = 0; row < BUFFER_ROWS; ++row) {
            VITIS_LOOP_32_3: for (int col = 0; col < IMAGE_WIDTH; ++col) {
                line_buf[row][col] = 0;
            }
        }

        VITIS_LOOP_37_4: for (int row = 0; row < KERNEL_SIZE; ++row) {
            VITIS_LOOP_38_5: for (int col = 0; col < KERNEL_SIZE; ++col) {
                window[row][col] = 0;
            }
        }

        VITIS_LOOP_43_6: for (int row = 0; row < IMAGE_HEIGHT; ++row) {
            const frontend_state_t state =
                (row < BUFFER_ROWS)
                    ? FRONTEND_FILL_BUFFER
                    : FRONTEND_PROCESS;

            VITIS_LOOP_49_7: for (int col = 0; col < IMAGE_WIDTH; ++col) {
                new_column[0] = line_buf[0][col];
                new_column[1] = line_buf[1][col];
                new_column[2] = input[channel][row][col];

                VITIS_LOOP_54_8: for (int window_row = 0;
                     window_row < KERNEL_SIZE; ++window_row) {
                    window[window_row][0] = window[window_row][1];
                    window[window_row][1] = window[window_row][2];
                    window[window_row][2] = new_column[window_row];
                }

                line_buf[0][col] = line_buf[1][col];
                line_buf[1][col] = input[channel][row][col];

                const bool process_enable =
                    state == FRONTEND_PROCESS &&
                    row >= KERNEL_SIZE - 1 &&
                    col >= KERNEL_SIZE - 1;

                if (process_enable && window_index < VALID_WINDOWS) {
                    VITIS_LOOP_70_9: for (int window_row = 0;
                         window_row < KERNEL_SIZE; ++window_row) {
                        VITIS_LOOP_72_10: for (int window_col = 0;
                             window_col < KERNEL_SIZE; ++window_col) {
                            windows[channel][window_index][window_row]
                                  [window_col] = window[window_row][window_col];
                        }
                    }
                    ++window_index;
                }
    ...
    VITIS_LOOP_86_11: for (int output_channel = 0;
         output_channel < OUTPUT_CHANNELS; ++output_channel) {
        VITIS_LOOP_88_12: for (int output_row = 0;
             output_row < IMAGE_HEIGHT; ++output_row) {
            VITIS_LOOP_90_13: for (int output_col = 0;
                 output_col < IMAGE_WIDTH; ++output_col) {
                accum_t accumulator = 0;
                const bool interior =
                    output_row > 0 && output_row < IMAGE_HEIGHT - 1 &&
                    output_col > 0 && output_col < IMAGE_WIDTH - 1;

                if (interior) {
                    const int index =
                        (output_row - 1) * VALID_COLS + output_col - 1;

                    VITIS_LOOP_101_14: for (int input_channel = 0;
                         input_channel < INPUT_CHANNELS; ++input_channel) {
                        VITIS_LOOP_103_15: for (int kernel_row = 0;
                             kernel_row < KERNEL_SIZE; ++kernel_row) {
                            VITIS_LOOP_105_16: for (int kernel_col = 0;
                                 kernel_col < KERNEL_SIZE; ++kernel_col) {
                                accumulator +=
                                    accum_t(windows[input_channel][index]
                    ...
                    VITIS_LOOP_117_17: for (int input_channel = 0;
                         input_channel < INPUT_CHANNELS; ++input_channel) {
                        VITIS_LOOP_119_18: for (int kernel_row = 0;
                             kernel_row < KERNEL_SIZE; ++kernel_row) {
                            VITIS_LOOP_121_19: for (int kernel_col = 0;
                                 kernel_col < KERNEL_SIZE; ++kernel_col) {
                                const int input_row =
